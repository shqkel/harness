...
> **このリポジトリは [revfactory/harness](https://github.com/revfactory/harness) のフォークです。** 上流の v1.2.0 を基点に、スペック追跡・命名規則・ハーネスポインタを追加しています。正確な差分は [フォークでの変更点](#フォークでの変更点--上流に対して何を追加したか) を参照してください。
...
## フォークでの変更点 — 上流に対して何を追加したか

**上流:** [revfactory/harness](https://github.com/revfactory/harness) v1.2.0 (`main` @ `cceac68`, 2026-06-10) · **このフォーク:** v1.4.1 · 全履歴は [CHANGELOG.md](CHANGELOG.md)。

以下の数値はフォーク時点ではなく、**上流の現在の `main` と比較した値**です。

### 1. スペック追跡 (`references/spec-tracking.md` 新規 · SKILL.md §5-6, §0-1)

上流には実行履歴を残す仕組みがありません。セッションが終わると *何を・なぜ・どの入力で・どこまで* 行ったかは会話ログにしか残らず、会話ログはプロジェクトの外にあります。このフォークは GitHub Spec Kit の**ドキュメントモデルのみ**を取り入れます — 番号付きスペックディレクトリ (`NNN-slug/`)、`specs/index.md` 台帳、`Draft → In Progress → Done → Superseded` のライフサイクル。ツールチェーンは導入しません(ブランチ単位の前提と implement 段階の二重化がハーネスのパイプラインと衝突するため)。

- **§5-6** — 新規に生成するすべてのハーネスにスペックを組み込み、オーケストレーターの開始・終了フェーズにフックを置きます。
- **§0-1** — 同じ規則をハーネス構築そのものに適用します。メタスキルが自身の構築・修正作業からスペックを開き、範囲が曖昧なら Draft で止めて合意を取ります。
- スペックは**プロジェクトフォルダ直下の `specs/`** に置きます(ハーネス配下は禁止)。1 つのプロジェクトが複数のハーネスを使うため採番は一系列である必要があり、ハーネスの区別は台帳の「ハーネス」列が担います。

### 2. 命名規則 (SKILL.md §2-4)

上流はエージェント・スキルのパスを `{name}.md`・`skill-name/` と書くだけで、`{name}` が何であるべきかを定めていません。`.claude/agents/`・`.claude/skills/` はプロジェクト内のすべてのハーネスが共有する名前空間なので、接頭辞がないと名前が衝突し、ファイル一覧だけでは所属がわかりません。

- エージェントは `{harness}-{role}.md`、スキルは `{harness}.{action}/SKILL.md`、frontmatter の `name` も同じパターン。接頭辞の略記は禁止します。
- スキル定義ファイル名は大文字の `SKILL.md` に固定 — 小文字の `skill.md` は大文字小文字を区別しないファイルシステムでのみ通り、他の環境では静かに読み込まれません。
- §6-1 は glob ではなく `ls` の実表記で検査します。macOS では glob が小文字も通してしまうためです。

### 3. ハーネスポインタと `$HARNESS_ROOT` (SKILL.md §5-4)

上流は CLAUDE.md のポインタを構築時に 1 回だけ登録します。**実行のたびに出力フォルダが変わる**ハーネスには足りません — 新しいフォルダを開いた次のセッションは、そのハーネスの存在を知りません。

- ポインタを **(a) ハーネスのホーム**(構築時に 1 回)と **(b) 出力プロジェクトフォルダ**(**実行のたび**)に分け、(b) は手順ではなくスペックフックの隣に置くオーケストレーターのフックとして実装しました。
- (b) には原本の位置を `$HARNESS_ROOT/{harness}/` と記します。コピーとなった `.claude/` が原本を辿れる必要があり、パスをリテラルで埋め込むと原本が移動した瞬間に嘘になるからです。未設定なら推測せず尋ねます。

### 4. 中間成果物フォルダ: `_workspace/` → `.{harness}/` (SKILL.md §5-1)

上流は中間成果物を共用の `_workspace/` に書きます。1 つの作業ディレクトリで 2 つのハーネスが動くと互いに上書きします。このフォークはハーネスごとに自分の名前を持つ隠しフォルダを与えます。`SKILL.md`・`orchestrator-template.md`・`team-examples.md`・`skill-testing-guide.md` に一括適用(上流で 41 箇所)。

### 5. Phase 7 の分離 (`references/harness-evolution.md` 新規)

上記の追加により `SKILL.md` がスキル自身の定める 500 行の上限(§4-4)を超えました。Phase 7(ハーネスの進化)は実行後・保守時にのみ必要な条件付きの内容なので、分離の第一候補です。本文には要約表とポインタを残し、他の文書が「メタスキル 7-3」として参照するため節番号は維持しています。

### 6. フォークのメタデータ (v1.4.1)

`owner`・`author`・`homepage`・`repository` が上流を指していたため、このフォークにしか存在しない v1.3.1 以降の Issue や PR を誤ったリポジトリへ送る状態でした。フォーク基準に修正し、`LICENSE` の上流著作権表示(`Copyright 2025 robin`、Apache-2.0)はそのまま残します。
...
| `references/spec-tracking.md` | — | 168 | 新規 |
| `references/harness-evolution.md` | — | 81 | 新規 |

**上流のままのもの:** 6 種類のチームアーキテクチャパターン、エージェントチーム優先の実行モデル、QA エージェントガイド、スキル作成・テストガイド(上記の命名ポインタを除く)、ライセンス。

**正確を期すための注記:** このフォークの CHANGELOG は 3-0/4-0 の重複レビュー段階と再利用設計セクションを 1.3.2 の追加分として記載しています。上流の現在の `main` にも存在するため、上の差分には**含まれません**。

## ライセンス

Apache 2.0 — 上流の著作権表示(`Copyright 2025 robin`)を維持。[LICENSE](LICENSE) を参照。
