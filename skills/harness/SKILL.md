...
3. **CLAUDE.md에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 원본 위치 + 변경 이력)만 기록한다. 하네스 홈에는 구축 시 1회, 출력 프로젝트 폴더에는 **실행마다** 심는다 (5-4).
...
2. 현황에 따라 실행 모드를 분기한다:
   ...
   - **운영/유지보수**: 기존 하네스의 감사·수정·동기화 요청 → `references/harness-evolution.md`의 7-5 운영/유지보수 워크플로우로 이동
...
5. **이 구축·수정 작업 자체를 스펙으로 연다** (0-1)

#### 0-1. 스펙 우선 — 하네스 구축도 스펙 아래에서 한다

하네스는 생성할 하네스에 스펙 추적을 심는 데(5-6) 그치지 않고, **자신의 구축·수정 작업부터 스펙 우선으로 수행한다.** 하네스 구축은 여러 세션에 걸쳐 반복되는데, 어떤 범위와 수용 기준 아래에서 그렇게 정했는지가 남지 않으면 다음 수정이 앞선 결정을 모르고 되돌린다. CLAUDE.md 변경 이력(7-3)은 *무엇이 바뀌었나*를 남기지만 *무엇을 하기로 하고 무엇을 하지 않기로 했나*는 남기지 않는다 — 그 자리가 스펙이다.

규격은 `references/spec-tracking.md`를 그대로 따른다. 위치와 게이트만 여기서 정한다:

- **위치** — 하네스가 사는 프로젝트 폴더 최상위 `specs/`. 하네스 원본을 `$HARNESS_ROOT`에 모아 관리하면 `$HARNESS_ROOT/specs/`, 프로젝트 안에서 직접 만드는 하네스면 `{프로젝트}/specs/`. 하네스 경계를 넘는 작업(상류를 A 하네스에서 고치고 하류를 B에 남기는 등)이 흔하므로 대장은 폴더당 하나이고, 하네스 구분은 대장의 「하네스」 컬럼이 맡는다
- **게이트** — 신규 구축·아키텍처 변경은 **Draft로 개설**하여 범위(무엇을 만들고 무엇을 안 만드는지)와 수용 기준을 합의한 뒤 In Progress로 전환한다. 에이전트 1개 추가처럼 발주가 명확하면 In Progress로 바로 연다
- **Phase 경계마다** 실행 로그에 한 줄 남긴다 (생성·수정한 파일 경로와 결과)
- **닫기** — Phase 6 검증 통과 후 완료 기록 → Done → index 갱신. Phase 7-3의 CLAUDE.md 변경 이력 기록과 짝으로 수행한다
...
에이전트·스킬 이름에는 **하네스 이름을 접두사로 붙인다.** `.claude/agents/`와 `.claude/skills/`는 한 프로젝트의 모든 하네스가 공유하는 네임스페이스다. 접두사가 없으면 `writer.md`·`reviewer.md` 같은 이름이 하네스 간에 충돌하고, 파일 목록만 봐서는 소속을 알 수 없어 현황 감사(Phase 0)와 중복 검토(3-0·4-0)가 이름만으로는 성립하지 않는다.

| 대상 | 패턴 | 예시 |
|------|------|------|
| 하네스 폴더 | `{harness}/` | `rapid-learner/` |
| 에이전트 파일 | `{harness}-{role}.md` | `rapid-learner-curriculum-planner.md` |
| 에이전트 frontmatter `name` | `{harness}-{role}` | `rapid-learner-curriculum-planner` |
| 스킬 폴더 | `{harness}.{action}/` | `rapid-learner.orchestrate/` |
| 스킬 frontmatter `name` | `{harness}.{action}` | `rapid-learner.orchestrate` |
| 스킬 정의 파일 | `SKILL.md` (대문자 고정) | `rapid-learner.orchestrate/SKILL.md` |
| 참조 문서 | 자유 명명 | `references/moscow-framework.md` |
...
- **접두사를 약어로 줄이지 않는다.** `case-study-to-channels` → `cs-writer`처럼 축약하면 접두사의 목적인 식별이 사라지고, 다른 하네스의 약어와 다시 충돌한다
- `{harness}/`(`.{action}` 없음)는 **진입 스킬**의 이름으로 쓴다 — 사용자가 직접 부르는 오케스트레이터다. 스킬이 하나뿐이면 그것이 진입 스킬이고, 여럿이면 진입 스킬 하나만 `{harness}/`를 갖고 나머지는 `{harness}.{action}/`이다. 접두사가 이미 붙어 있으므로 충돌하지 않는다
- 스킬 정의 파일명은 `SKILL.md`로 **고정**한다 — 대소문자를 구분하는 파일시스템에서 `skill.md`는 로딩되지 않는다. macOS처럼 대소문자를 무시하는 환경에서 통과하다가 배포·공유 시점에 드러나는 종류의 결함이다
- 기존 하네스는 일괄 소급하지 않는다 — **다음에 실제로 손대는 시점에** 그 하네스의 이름을 규칙에 맞춘다 (스펙 추적과 같은 원칙, 5-6)
...
**모든 에이전트는 반드시 `프로젝트/.claude/agents/{harness}-{role}.md` 파일로 정의한다**(명명 규칙은 2-4). 에이전트 정의 파일 없이 Agent 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
...
각 에이전트를 `프로젝트/.claude/agents/{harness}-{role}.md`에 정의한다. 필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업. 에이전트 팀 모드에서는 `## 팀 통신 프로토콜` 섹션을 추가하여 메시지 수신/발신 대상과 작업 요청 범위를 명시한다.
...
각 에이전트가 사용할 스킬을 `프로젝트/.claude/skills/{harness}.{action}/SKILL.md`에 생성한다(명명 규칙은 2-4). 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.
...
파일 기반 전달 시 규칙:
- 작업 디렉토리 하위에 `.<하네스명>/` 폴더를 만들어 중간 산출물 저장. `.<하네스명>`은 **해당 하네스의 실제 이름을 붙인 hidden 폴더**다(예: rapid-learner → `.rapid-learner/`, tinywins-video-lesson → `.tinywins-video-lesson/`). 하네스마다 폴더 이름이 달라 여러 하네스가 한 작업 디렉토리에서 충돌 없이 공존한다. (구 관례였던 공용 `_workspace/`는 2026-07 폐기)
- 파일명 컨벤션: `{phase}_{agent}_{artifact}.{ext}` (예: `01_analyst_requirements.md`)
- 최종 산출물만 사용자 지정 경로에 출력, 중간 파일(`.<하네스명>/`)은 보존 (사후 검증·감사 추적용)
...
CLAUDE.md는 새 세션마다 자동 로딩되는 유일한 파일이다. 하네스의 존재와 트리거 규칙이 여기 없으면 오케스트레이터 스킬이 트리거되지 않고, 사용자가 매번 스킬 이름을 직접 불러야 한다.

포인터를 심는 자리가 **둘이고, 시점이 다르다.**

**(a) 하네스 홈 `{harness}/CLAUDE.md` — 하네스 구축 시 1회**

하네스 원본이 사는 폴더. 하네스 자체의 정체성·경계·진화를 기록한다.
...
**트리거:** {도메인} 관련 작업 요청 시 `{harness}.{action}` 스킬을 사용하라. 단순 질문은 직접 응답 가능.

**경계:** {이 하네스가 하지 않는 것과, 그 일을 맡는 다른 하네스}
...
**(b) 출력 프로젝트 폴더 `{출력 경로}/CLAUDE.md` — 하네스를 실행할 때마다**

산출물이 놓이는 폴더. 하네스는 실행마다 **다른 프로젝트 폴더**에서 돌 수 있으므로, 이 포인터는 구축 시 1회가 아니라 **매 실행에 필요하다.** 따라서 절차가 아니라 오케스트레이터의 훅으로 박는다 — 스펙 훅(5-6)과 같은 자리다:

- **시작 Phase**: 출력 경로의 기존 `CLAUDE.md`와 `specs/index.md`를 읽는다. 있으면 이어 쓰고, 없으면 만든다
- **최종 보고 Phase**: 이번 실행 내용을 변경 이력에 한 줄 추가한다

````markdown
## 하네스: {도메인명}

**원본:** `$HARNESS_ROOT/{harness}/` — 규칙·에이전트·스킬의 권위는 원본에 있다. 이 폴더의 `.claude/`는 사본이며, 사본에서 한 개선이 하네스 일반에 해당하면 원본에 반영한다.

**트리거:** {도메인} 관련 작업 요청 시 `{harness}.{action}` 스킬을 사용하라.

**산출물:** 최종 `{최종 산출물 경로}` · 중간 `.{harness}/` · 실행 이력 `specs/`
...
**`$HARNESS_ROOT` 해석 규칙.** 하네스 원본 모음의 루트는 환경변수 `HARNESS_ROOT`로 가리킨다. 경로는 사용자마다 다르고, 문서에 리터럴로 박으면 원본이 옮겨지는 순간 거짓이 된다.

1. 설정돼 있으면 그 값을 쓴다 — `echo "$HARNESS_ROOT"`로 확인
2. 미설정이면 **경로를 추측하지 말고** 사용자에게 한 번 묻는다. CLAUDE.md에는 확인받은 경로를 리터럴로 적지 말고 `$HARNESS_ROOT` 표기를 유지하되, 답변에서 "`HARNESS_ROOT`를 `{경로}`로 설정하면 이후 세션에서 자동 해석된다"고 안내한다
3. 원본 모음 없이 프로젝트 안에서 직접 만든 하네스는 **원본** 줄에 "이 프로젝트 — `.claude/` 가 원본"이라고 적는다. 사본이 아니므로 가리킬 상류가 없다

**CLAUDE.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.claude/agents/`, `.claude/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. CLAUDE.md는 **포인터(트리거·원본·산출물) + 변경 이력**만 담는다.
...
- `.<하네스명>/` 존재 + 사용자가 부분 수정 요청 → **부분 재실행** (해당 에이전트만 재호출)
- `.<하네스명>/` 존재 + 사용자가 새 입력 제공 → **새 실행** (기존 .<하네스명>를 `.<하네스명>_prev/`로 이동)
- `.<하네스명>/` 미존재 → **초기 실행**
...
- **명명 규칙 검증 (2-4)** — 에이전트 파일·frontmatter `name`이 `{harness}-{role}`인지, 스킬 폴더·frontmatter `name`이 `{harness}.{action}`인지, 스킬 정의 파일명이 대문자 `SKILL.md`인지. 대소문자 무시 파일시스템에서는 `ls`의 실제 표기로 확인한다 (glob은 소문자 `skill.md`도 통과시킨다)
...
| 절 | 내용 |
|----|------|
| 7-1 | 실행 후 피드백 수집 — 매 실행 완료 후 개선점을 묻는다. 강요하지 않되 반드시 기회를 준다 |
| 7-2 | 피드백 반영 경로 — 피드백 유형별 수정 대상 (결과 품질→스킬, 역할→에이전트 정의, 순서→오케스트레이터, 트리거→description) |
| 7-3 | 변경 이력 — 모든 변경을 CLAUDE.md 변경 이력 테이블(5-4)에 기록한다. 퇴행 방지의 유일한 장치 |
| 7-4 | 진화 트리거 — 같은 피드백 2회 반복·반복 실패 패턴·오케스트레이터 우회가 관찰되면 먼저 제안한다 |
| 7-5 | 운영/유지보수 워크플로우 — Phase 0의 "운영/유지보수" 분기가 진입하는 4단계 (감사 → 점진 수정 → 동기화 → 검증) |

> 각 절의 상세·표·템플릿은 `references/harness-evolution.md` 참조. 실행 후 피드백을 받았거나 Phase 0에서 **운영/유지보수** 분기로 들어왔으면 이 파일을 읽는다.
...
- [ ] **명명 규칙 준수** — 에이전트 `{harness}-{role}.md`, 스킬 `{harness}.{action}/SKILL.md`, 중간 산출물 `.{harness}/` (2-4)
- [ ] **구축 작업 자체를 스펙으로 개설·완료 기록** — `specs/` 대장 등재 → Done (0-1)
...
- [ ] **하네스 홈 CLAUDE.md에 포인터 등록** (트리거 + 경계 + 변경 이력) (5-4a)
- [ ] **출력 프로젝트 CLAUDE.md 포인터 훅**이 오케스트레이터의 시작/최종 보고 Phase에 있음 — `$HARNESS_ROOT/{harness}/` 원본 표기 포함 (5-4b)
...
- **하네스 진화 가이드**: `references/harness-evolution.md` — 실행 후 피드백 반영, 변경 이력, 운영/유지보수 워크플로우 (Phase 7 상세)
