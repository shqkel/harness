...
### Phase 0: 컨텍스트 확인 + 스펙 개설 (후속 작업 지원)
...
5. 사용자에게 결과 요약 보고 (스펙 번호를 밝힌다. 단 **최종 산출물에는 스펙 번호를 노출하지 않는다** — 스펙은 내부 산출물)
...
### Phase 0: 컨텍스트 확인 + 스펙 개설
(Template A와 동일 — `.<하네스명>/` 존재 여부 분기 + 프로젝트 폴더 최상위 `specs/`에 스펙 개설/귀속)
...
3. 결과 요약 보고 (스펙 번호는 보고에만, 최종 산출물에는 넣지 않는다)
