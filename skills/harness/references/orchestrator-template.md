...
### Phase 0: 컨텍스트 확인 + 스펙 개설 + 포인터 확인 (후속 작업 지원)

기존 산출물 존재 여부를 확인하여 실행 모드를 결정한다:

1. `.<하네스명>/` 디렉토리 존재 여부 확인
2. 실행 모드 결정:
   - **`.<하네스명>/` 미존재** → 초기 실행. Phase 1로 진행
   - **`.<하네스명>/` 존재 + 사용자가 부분 수정 요청** → 부분 재실행. 해당 에이전트만 재호출하고, 기존 산출물 중 수정 대상만 덮어쓴다
   - **`.<하네스명>/` 존재 + 새 입력 제공** → 새 실행. 기존 `.<하네스명>/`를 `.<하네스명>_{YYYYMMDD_HHMMSS}/`로 이동한 뒤 Phase 1 진행
3. 부분 재실행 시: 이전 산출물 경로를 에이전트 프롬프트에 포함하여, 에이전트가 기존 결과를 읽고 피드백을 반영하도록 지시
4. **스펙 개설/귀속** — 프로젝트 폴더 **최상위** `specs/index.md`를 읽고(하네스 하위가 아니다) 이번 요청을 귀속 규칙에 따라 기존 스펙에 붙이거나 새 스펙(`NNN-slug/spec.md`)을 개설한다.
   대장은 전 하네스 공용이므로 **이미 있으면 다음 번호를 이어 쓰고** 「하네스」 컬럼에 이 하네스를 표기한다. `specs/`가 없으면 이때 만든다(소급 없음).
   발주가 명확하면 In Progress로 바로 개설하고, 범위 해석이 갈리면 Draft로 두고 확인을 받는다. 규격: `references/spec-tracking.md`
   > 이후 **Phase 경계마다** 스펙의 실행 로그에 한 줄씩 남긴다 (산출물 경로·결과 요약).
5. **출력 폴더 CLAUDE.md 확인/개설** — 출력 경로의 `CLAUDE.md`를 읽는다. 없으면 이 하네스의 포인터 절을 만든다 (원본 `$HARNESS_ROOT/<하네스명>/` · 트리거 · 산출물 경로). 실행마다 출력 폴더가 달라지므로 구축 시 1회가 아니라 **매 실행에 확인**한다. 템플릿: 하네스 메타스킬 §5-4(b)

### Phase 1: 준비
1. 사용자 입력 분석 — {무엇을 파악하는지}
2. 작업 디렉토리에 `.<하네스명>/` 생성
   - **초기 실행**: 새 `.<하네스명>/` 생성
   - **새 실행**: 기존 `.<하네스명>/`를 `.<하네스명>_{YYYYMMDD_HHMMSS}/`로 이동한 직후 새 `.<하네스명>/` 재생성
3. 입력 데이터를 `.<하네스명>/00_input/`에 저장
...
| 팀원 | 출력 경로 |
|------|----------|
| {teammate-1} | `.<하네스명>/{phase}_{teammate-1}_{artifact}.md` |
| {teammate-2} | `.<하네스명>/{phase}_{teammate-2}_{artifact}.md` |
...
### Phase 5: 정리 + 스펙 마감 + 포인터 갱신
1. 팀원들에게 종료 요청 (SendMessage)
2. 팀 정리 (TeamDelete)
3. `.<하네스명>/` 디렉토리 보존 (중간 산출물은 삭제하지 않음 — 사후 검증·감사 추적용)
4. **스펙 마감** — 완료 기록(산출물 목록·검증 결과·남긴 것) 작성 → 상태 `Done` → `specs/index.md` 갱신
5. **출력 폴더 CLAUDE.md 갱신** — 변경 이력에 이번 실행을 한 줄 추가 (날짜·내용·대상·사유)
6. 사용자에게 결과 요약 보고 (스펙 번호를 밝힌다. 단 **최종 산출물에는 스펙 번호를 노출하지 않는다** — 스펙은 내부 산출물)

> **팀 재구성이 필요한 경우:** Phase별로 다른 전문가 조합이 필요하면, 현재 팀을 TeamDelete로 정리한 뒤 새 TeamCreate로 다음 Phase의 팀을 구성한다. 이전 팀의 산출물은 `.<하네스명>/`에 보존되므로 새 팀이 Read로 접근 가능.
...
### Phase 0: 컨텍스트 확인 + 스펙 개설 + 포인터 확인
(Template A와 동일 — `.<하네스명>/` 존재 여부 분기 + 프로젝트 폴더 최상위 `specs/`에 스펙 개설/귀속 + 출력 폴더 `CLAUDE.md` 확인/개설)

### Phase 1: 준비
1. 입력 분석
2. `.<하네스명>/` 생성 (초기 실행 시, 또는 새 실행에서 기존 `.<하네스명>/`를 보관 디렉토리로 이동한 직후)
...
|---------|------|------|-------|-------------------|
| {agent-1} | {소스} | `.<하네스명>/{phase}_{agent}_{artifact}.md` | opus | true |
| {agent-2} | {소스} | `.<하네스명>/{phase}_{agent}_{artifact}.md` | opus | true |
...
### Phase 4: 정리 + 스펙 마감 + 포인터 갱신
1. `.<하네스명>/` 보존
2. 스펙 완료 기록 → 상태 `Done` → `specs/index.md` 갱신
3. 출력 폴더 `CLAUDE.md` 변경 이력에 이번 실행 한 줄 추가
4. 결과 요약 보고 (스펙 번호는 보고에만, 최종 산출물에는 넣지 않는다)
...
각 결과는 `.<하네스명>/02_{agent}_raw.md`에 저장.

### Phase 3: 합의 기반 통합
**실행 모드:** 에이전트 팀

1. `TeamCreate`로 통합 팀 구성 (editor + fact-checker + synthesizer)
2. `TaskCreate`로 작업 분배 — 모두 Phase 2의 `.<하네스명>/02_*` 파일을 Read
3. 팀원들이 `SendMessage`로 상충 데이터를 논의, 파일 기반으로 합의안 도출
4. 최종 통합본 `.<하네스명>/03_integrated.md` 생성
5. `TeamDelete`로 팀 정리

### Phase 4: 독립 검증
**실행 모드:** 서브 에이전트

단일 QA 서브 에이전트가 `.<하네스명>/03_integrated.md`를 입력으로 받아 검증 보고서 생성.
```

**스펙 훅:** 하이브리드도 예외가 아니다 — **첫 Phase**에서 프로젝트 폴더 최상위 `specs/`에 스펙을 개설/귀속하고, **마지막 Phase**에서 완료 기록 → `Done` → index 갱신을 수행한다. 모드 전환 지점(팀↔서브)도 Phase 경계이므로 실행 로그에 한 줄 남긴다.

**포인터 훅:** 스펙 훅과 같은 자리에 붙인다 — 첫 Phase에서 출력 폴더 `CLAUDE.md` 확인/개설, 마지막 Phase에서 변경 이력 한 줄 추가.
...
4. **파일 경로는 절대적으로** — 상대 경로 금지, `.<하네스명>/` 기준 명확한 경로
...
8. **스펙 훅을 시작·종료 Phase에 반드시** — 시작 Phase에 "스펙 개설/귀속", 종료 Phase에 "완료 기록 → Done → index 갱신". 스펙은 프로젝트 폴더 **최상위** `specs/`에 두고(하네스 하위 금지) 번호는 프로젝트당 한 수열로 이어 쓴다. 훅이 빠지면 실행 이력이 대화 로그에만 남아 세션 종료와 함께 사실상 소실된다 (`references/spec-tracking.md`)
9. **출력 폴더 CLAUDE.md 포인터 훅도 시작·종료 Phase에** — 시작 Phase에 "출력 폴더 `CLAUDE.md` 확인/개설(원본 `$HARNESS_ROOT/<하네스명>/` 표기 포함)", 종료 Phase에 "변경 이력 한 줄 추가". 하네스 구축 시 1회로는 부족하다 — **실행마다 출력 폴더가 달라지므로**, 훅이 없으면 새 폴더에서 다음 세션이 이 하네스의 존재를 모른다 (하네스 메타스킬 §5-4)
